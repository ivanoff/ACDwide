...
use SimplenessCMS::DATABASE;
use SimplenessCMS::TRANSLATE;
...
use lib '.';
use SimplenessCMS::CGI;
our $q = SimplenessCMS::CGI->new( { Directory => $CONFIG->{session_dir}, Session_enabled => 0, } );
$SESSION = $q->{Session};
...
$t = SimplenessCMS::TRANSLATE->new($lang);
$db = SimplenessCMS::DATABASE->new;
