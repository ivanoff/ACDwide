...
{
    package SimplenessCMS::CGI::Session;
    our @ISA = qw( SimplenessCMS::CGI );
    sub new {
        my ( $class, $params ) = @_;
        $params ||= {};
        my $self = $params;
...
#$session    = new SimplenessCMS::CGI::Session(undef, $sid, {Directory=>'/tmp'});
...
####
    $self->{Session} = SimplenessCMS::CGI::Session->new( $params );
